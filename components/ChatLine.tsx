...
const findHouse = (text: string) => {
  const regex = /(Ravenclaw|Griffindor|Hufflepuff|Slytherin)/;
...
export function ChatLine({ role = "assistant", content }: ChatGPTMessage) {
  ...
  const hogwartsHouse = findHouse(content);
      ...
      <div className="float-right mb-5 rounded-lg bg-zinc-800 px-4 py-5 shadow-lg ring-1 ring-zinc-700 sm:px-6">
        <div className="flex space-x-3">
          <div className="flex-1 gap-4">
            ...
            <p className="font-normal">{formatteMessage}</p>
            {hogwartsHouse && role === "assistant" && (
              <HouseCard house={hogwartsHouse} />
            )}
          </div>
        </div>
      </div>
